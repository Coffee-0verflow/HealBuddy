...
export default function SOSButton({ compact = false }) {
  ...
  const fetchAndShare = async () => {
    ...
    try {
      ...
      const built = buildMessage(lat, lng);
      setMsg(built);
      ...
      if (navigator.share) {
        ...
        // After native share closes, show the panel as backup
        setOpen(true);
      } else {
        setOpen(true);
      }
      ...
      if (err?.code === 1) {
        ...
        // User cancelled native share — still show panel
        setState(STATES.ready);
        setOpen(true);
      } else {
        setState(STATES.error);
  ...
  const reset = () => { setState(STATES.idle); setMsg(null); setOpen(false); setCopied(false); };
  ...
  return (
    <div className="w-full">
      ...
      <button
        onClick={fetchAndShare}
        disabled={state === STATES.loading}
        className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl font-black text-base shadow-lg active:scale-[0.97] transition-all disabled:opacity-70
      ...
      {state === STATES.error && (
        <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-xl flex items-start gap-2">
          ...
          <div className="flex-1">
            <p className="text-xs font-bold text-red-800 dark:text-red-300">Unable to fetch location.</p>
            <p className="text-xs text-red-600 dark:text-red-400 mt-0.5">Please enable location services and try again.</p>
          </div>
          <button onClick={reset} className="text-red-400 hover:text-red-600 text-xs font-bold shrink-0">Retry</button>
        </div>
      )}
